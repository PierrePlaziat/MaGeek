...
namespace MaGeek.UI
{
    ...
    public partial class DeckList : TemplatedUserControl
    {
        ...
        {
            string title = Dialogs.GetInpurFromUser("What title?","New title");
        ...
        private async void RenameDeck(object sender, RoutedEventArgs e)
        {
            ...
            string title = Dialogs.GetInpurFromUser("What title?", "New title");
            await MageekService.MageekService.RenameDeck(App.State.SelectedDeck.DeckId, title);
            await Reload();
        }
        ...
        {
            if (App.State.SelectedDeck == null) return;
            var totalPrice = await MageekService.MageekService.EstimateDeckPrice(App.State.SelectedDeck.DeckId, App.Config.Settings[UserSetting.Currency]);
